 ...
 * `var(--actions-x, <literal>)` and the sheet declares no `--actions-*`
 ...
 * property the table does not document, and a `--actions-*` DECLARED in the
 * sheet.
 */
...
function parseReadmeTable(markdown: string): Map<string, string> {
  ...
  const row = /^\|\s*`(--actions-[\w-]+)`\s*\|\s*`(.+?)`\s*\|/gm;
...
describe('the seam is what the README says it is', () => {
  it('parses the published table', () => {
    expect(documented.size).toBe(9);
    expect(documented.get('--actions-gap')).toBe('1rem');
    expect(documented.get('--actions-direction')).toBe('row');
  });
  ...
  it('declares no --actions-* property anywhere', () => {
    const declarations = [...body.matchAll(/(?:^|[;{\s])(--actions-[\w-]+)\s*:/g)].map(
      (match) => match[1],
    );
...
describe('the sanity rules the sheet is written under', () => {
  ...
  it('descends every block rule from the root, except the editor chrome', () => {
    ...
    for (const selector of selectors) {
      ...
      expect(
        selector.startsWith('.actions-block') || selector === '.actions-notice',
        selector,
      ).toBe(true);
    }
  });
